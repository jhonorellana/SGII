...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_PATRIMONIO_CONSOLIDADO`(IN `p_fecha_inicio` VARCHAR(10), IN `p_fecha_fin` VARCHAR(10), IN `p_id_grupo_familiar` INT, IN `p_id_propietario` INT, IN `p_incluir_dividendos` TINYINT(1))
BEGIN
    -- Intereses esperados
    ...
    SELECT 'Acciones Sin Interés' as detalle,
           COALESCE(SUM(
               CASE 
                   WHEN p_incluir_dividendos = 1 THEN
                       CASE
