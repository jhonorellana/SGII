      ...
      next: (response) => {
        if (response.success && response.data) {
          ...
          this.ventas = ventasFiltradas.map((venta: any) => {
            ...
            let vNominal = venta.valor_nominal || venta.valorNominalTotal || 0;
            if ((!vNominal || vNominal === 0) && venta.detalles && venta.detalles.length > 0) {
              vNominal = venta.detalles.reduce((sum: number, d: any) => sum + Number(d.valor_nominal || 0), 0);
            }

            // Failsafe para garantizar métricas duales (Venta vs Total Return)
            let capVendido = Number(venta.valor_venta_con_comision || 0) - Number(venta.utilidad_con_comision || 0);
            if (capVendido <= 0 && venta.detalles && venta.detalles[0] && Number(venta.detalles[0].valor_compra) > 0) {
              capVendido = Number(venta.detalles[0].valor_compra);
            }
            if (capVendido <= 0 && venta.inversion && Number(venta.inversion.capital_invertido) > 0) {
              capVendido = Number(venta.inversion.capital_invertido);
            }
